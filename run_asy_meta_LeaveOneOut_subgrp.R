...
## For additional analyses in R2
## get the dataset list of each subgroup
#subgrp_dataset = dataset_info[dataset_info$ScannerField0=='1.5',]$Dataset
#subgrp_str = 'Scanner1.5'
#subgrp_dataset = dataset_info[dataset_info$ScannerField0=='3',]$Dataset
#subgrp_str = 'Scanner3'
#subgrp_dataset = dataset_info[dataset_info$AgeMax<=18,]$Dataset
#subgrp_str = 'Child'
#subgrp_dataset = dataset_info[dataset_info$AgeMin>18,]$Dataset
#subgrp_str = 'Adult'
subgrp_dataset = dataset_info[!dataset_info$Dataset %in% c('BIG', 'QTIM'),]$Dataset
subgrp_str = 'exclBIGQTIM'

if(!file.exists(file.path(sumout_dir, subgrp_str))){
  dir.create(file.path(sumout_dir, subgrp_str))
}

#for (dataset_index in 1:length(dataset_info$Dataset)){
#  dataset = dataset_info$Dataset[dataset_index]
for (dataset_index in 1:length(subgrp_dataset)){
  dataset = subgrp_dataset[dataset_index]
  for (i in 1:length(csv_file_list)){
    ...
    csv_dat = csv_dat[csv_dat$Dataset %in% subgrp_dataset[subgrp_dataset!=dataset],]
    
    res = rma(yi=yi, vi=vi, data = csv_dat, control = list(maxiter=500)) # run the meta analysis with random effect modelling. 
  ...
  csv_file = paste(sumout_dir, subgrp_str, paste('sum_asy_meta_excl_', dataset,'.csv',sep=''), sep='/')
